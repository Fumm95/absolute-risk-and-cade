##################################################################
####### 4. Projecting Absolute Risk of Disease Mortality #########
##################################################################

# Calculating Mortality Age Groups and Yearly Mortality Risk

## EUR
bc_white_mort = bc_mortality %>% filter(Race == "White") # Mortality rate from SEER

### Calculating next 5 year risk of disease incidence
...
## AFR
bc_black_mort = bc_mortality %>% filter(Race == "Black") # Mortality rate from SEER
...
## AMR
bc_hisp_mort = bc_mortality %>% filter(Race == "Hispanic") # Mortality rate from SEER
...
## ASN
bc_asn_mort = bc_mortality %>% filter(Race == "Asian") # Mortality rate from SEER
...
# Projecting 10 Year Risk of Disease Mortality
...
calcprob = function(df){
  ...
  for(i in c(1:9)){
    tot_inc = (df$abs_prob * min(i, 5) + df$abs_prob5 * (max(i, 5) - 5)) # Probability of disease incidence after i years
    tot_mort = (df$bc_mortnow * (5 - min(i, 5)) + df$bc_mort5 * (10 - max(i, 5))) # Probability of disease mortality after i years
